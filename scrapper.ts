...
const data =
  await (await fetch("https://www.cinesunidos.com/Peliculas/Valencia")).text();
...
$(".col-md-3 ").each((_: any, val: any) => {
  ...
  const id = $(val).find(".mod_titulo_pelicula").find("a").attr("href").split(
    "/",
  ).at(-1);
...
async function saveImage(pelicula: Pelicula,ruta:string) {
  ...
  return { ...pelicula, img: `http://192.168.0.112:8000/poster/${filename}` };
}

if (online) {
  console.log("online:", online);
  const promise = peliculas.map((p)=> saveImage(p,'./static/poster'));
  const peli = await Promise.all(promise);
  Deno.writeTextFile(`./bd/peliculas.json`, JSON.stringify(peli), {
    create: true,
  });
} else {
